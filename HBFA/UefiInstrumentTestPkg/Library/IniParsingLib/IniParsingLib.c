  ...
  The INI file format is:
    ...
    Where:
      ...
      3) EntryValue can be:
         ...
         3.5) a EFI_STATUS value. The valid value is defined in UEFI specification. Such as EFI_SUCCESS, EFI_INVALID_PARAMETER, etc.
...
GLOBAL_REMOVE_IF_UNREFERENCED  EFI_STATUS_STRING mEfiStatusString[] = {
...
//{"EFI_WARN_RESET_REQUIRED",    EFI_WARN_RESET_REQUIRED},
};
  ...
  )
{
  ...
  for (Index = 0; Index < ARRAY_SIZE(mEfiStatusString); Index++) {
    if (AsciiStrCmp (String, mEfiStatusString[Index].String) == 0) {
      *Status = mEfiStatusString[Index].Status;
      return EFI_SUCCESS;
    }
  }
  return EFI_INVALID_PARAMETER;
...
/**
  ...
  @retval EFI_NOT_FOUND  Section is not found.
  @retval EFI_INVALID_PARAMETER  Section format is invalid.
**/
EFI_STATUS
EFIAPI
GetGuidFromDataFile (
...
/**
  Get section entry EFI_STATUS value.
  ...
  @retval EFI_SUCCESS    Section entry EFI_STATUS value is got.
  @retval EFI_NOT_FOUND  Section is not found.
  @retval EFI_INVALID_PARAMETER  Section format is invalid.
**/
EFI_STATUS
EFIAPI
GetEfiStatusFromDataFile (
  IN      VOID                          *Context,
  IN      CHAR8                         *SectionName,
  IN      CHAR8                         *EntryName,
  ...
  )
{
  ...
  if (Context == NULL || SectionName == NULL || EntryName == NULL || EfiStatus == NULL) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  ASSERT (Value != NULL);
  RStatus = AsciiStrToEfiStatus (Value, EfiStatus);
