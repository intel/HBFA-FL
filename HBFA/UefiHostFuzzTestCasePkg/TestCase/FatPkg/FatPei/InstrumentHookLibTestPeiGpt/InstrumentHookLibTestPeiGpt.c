  ...
  )
{
  ...
  if (FuncHook->CurrentCallCount == FuncHook->CallErrorCount) {
    ...
    if (FuncHook->Func == (UINTN)AllocatePages) {
      SetParameterValue (EntryContext, 1, (UINTN)-1);
    } else if (FuncHook->Func == (UINTN)FatReadBlock) {
      SetParameterValue64 (EntryContext, 2, PEI_FAT_MAX_BLOCK_DEVICE);
    }
    return 1;
  }
  return 0;
...
GLOBAL_REMOVE_IF_UNREFERENCED FUNC_HOOK mFuncHook[] = {
  {"AllocatePages",    (UINTN)AllocatePages,    (UINTN)CommonEnter, (UINTN)CommonExit},
  {"FatReadBlock",     (UINTN)FatReadBlock,     (UINTN)CommonEnter, (UINTN)CommonExit},
};
