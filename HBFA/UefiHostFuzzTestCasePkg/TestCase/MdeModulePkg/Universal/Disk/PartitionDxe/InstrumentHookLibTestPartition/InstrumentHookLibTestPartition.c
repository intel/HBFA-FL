  ...
  )
{
  ...
  if (FuncHook->CurrentCallCount == FuncHook->CallErrorCount) {
    ...
    } else if (FuncHook->Func == (UINTN)ReadBlocks) {
      SetParameterValue64 (EntryContext, 3, (UINT64)-1);
    } else if (FuncHook->Func == (UINTN)ReadDisk) {
      SetParameterValue64 (EntryContext, 3, (UINT64)-1);
    }
    return 1;
  }
...
GLOBAL_REMOVE_IF_UNREFERENCED FUNC_HOOK mFuncHook[] = {
  ...
  {"ReadBlocks",       (UINTN)ReadBlocks,       (UINTN)CommonEnter, (UINTN)CommonExit},
  {"ReadDisk",         (UINTN)ReadDisk,         (UINTN)CommonEnter, (UINTN)CommonExit},
};
