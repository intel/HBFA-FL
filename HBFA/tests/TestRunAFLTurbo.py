...
class TestRunAFLTurbo(unittest.TestCase):

    # Test bad value for arch is handled (-a 'X64')
    @mock.patch('UefiHostTestTools.RunAFLTurbo.Build', return_value='')
    @mock.patch('UefiHostTestTools.RunAFLTurbo.RunAFLTurbo', return_value='')
    @mock.patch('UefiHostTestTools.RunAFLTurbo.CheckTestEnv', return_value='')
    @mock.patch('UefiHostTestTools.RunAFLTurbo.HBFA_PATH',
                '/root/hbfa_workspace/edk2-staging/HBFA')
    @mock.patch('sys.argv',
                ['RunAFLTurbo.py', '-m', '/root/hbfa_workspace/edk2-'
        ...
        with io.StringIO() as buffer:
            with contextlib.redirect_stderr(buffer):
                with self.assertRaises(SystemExit):
                    UefiHostTestTools.RunAFLTurbo.main()
    ...
    @mock.patch('UefiHostTestTools.RunAFLTurbo.HBFA_PATH',
                '/root/hbfa_workspace/edk2-staging/HBFA')
    @mock.patch('sys.argv',
                ['RunAFLTurbo.py', '-m', '/tmp/TestBmpSupportLib.inf',
    ...
    @mock.patch('UefiHostTestTools.RunAFLTurbo.HBFA_PATH',
                '/root/hbfa_workspace/edk2-staging/HBFA')
    @mock.patch('sys.argv', ['RunAFLTurbo.py', '-m', '', '-i', '/tmp/', '-o',
    ...
    def test_args_bad_module_bad_value(self, *args, **kwargs):
        ...
        with io.StringIO() as buffer:
            ...
            check_string = b'\x4d\x6f\x64\x75\x6c\x65\x46\x69\x6c\x65\x20' \
                ...
                + b'\x74\x20\x69\x73\x20\x6e\x6f\x20\x65\x78\x69\x74\x73' \
                + b'\x20\x6f\x72\x20\x6e\x6f\x74\x20\x74\x68\x65\x20\x72' \
                + b'\x65\x6c\x61\x74\x69\x76\x65\x20\x70\x61\x74\x68\x20' \
                + b'\x66\x6f\x72\x20\x48\x42\x46\x41\x0a'
            check_string = check_string.decode("ascii")
            self.assertEqual(message, check_string)
    ...
    @mock.patch('UefiHostTestTools.RunAFLTurbo.Build', return_value='')
    @mock.patch('UefiHostTestTools.RunAFLTurbo.RunAFLTurbo', return_value='')
    @mock.patch('UefiHostTestTools.RunAFLTurbo.CheckTestEnv',
                return_value='')
    @mock.patch('UefiHostTestTools.RunAFLTurbo.HBFA_PATH',
                '/root/hbfa_workspace/edk2-staging/HBFA')
    @mock.patch('sys.argv',
                ['RunAFLTurbo.py', '-m',
                 ...
                 'Library/BaseBmpSupportLib/TestBmpSupportLib.inf',
                 '-o', '/tmp/', '-a', 'X64',
                 '-c', 'rawcommand', '-i', '/tmp/tmp/tmp/tmp'])
    @mock.patch('os._exit', sys.exit)
    def test_args_bad_input_seed_path(self, *args, **kwargs):
        ...
        with io.StringIO() as buffer:
            ...
            check_string = b'\x49\x6e\x70\x75\x74\x53\x65\x65\x64\x20\x70' \
                ...
                + b'\x2f\x74\x6d\x70\x2f\x74\x6d\x70\x20\x69\x73\x20\x6e' \
                + b'\x6f\x20\x65\x78\x69\x73\x74\x73\x0a'
            check_string = check_string.decode("ascii")
            self.assertEqual(message, check_string)
    ...
    @mock.patch('sys.argv',
                ['RunAFLTurbo.py', '-m',
                 ...
                 '-i', '/tmp/', '-o', '/tmp/test', '-a', 'X64',
                 '-c', 'gnome'])
    @mock.patch('UefiHostTestTools.RunAFLTurbo.SysType', 'Linux')
    @mock.patch('os._exit', sys.exit)
...
if __name__ == "__main__":
    ...
    import UefiHostTestTools.RunAFLTurbo
    unittest.main()
