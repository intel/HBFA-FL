...
def CheckTestEnv():
    ...
    if SysType == "Windows":
        print("AFLTurbo doesnt support running in Windows. Please try with "
              "linux.")
        os._exit(0)
...
def RunAFLTurbo(TestModuleBinPath, InputPath, OutputPath):
    ...
    AFL_CMD = []
    AFL_CMD.append(AFL_PATH+"/afl-turbo-fuzz")
    AFL_CMD.append("-i")
...
def main():
    ...
    parser.add_argument("-c", "--commandline", dest="CommandLine",
                        ...
                        "the AFLTurbo fuzzer after building the test "
                        "module and is recommended for automated "
                        "approaches. Alternatively, 'manual' may be used"
                        " to simply build the test case "
                        "and print out a command that the user can run to "
                        "start the fuzzer (this will preserve the fuzzing "
                        "display for AFLTurbo.")
    ...
    if not args.ModuleFile:
        ...
        if args.ModuleFile.startswith(HBFA_PATH):
            ...
            print("ModuleFile path: {}".format(args.ModuleFile) +
                  " should be start with {}.".format(HBFA_PATH))
            os._exit(0)
    elif not os.path.exists(os.path.join(HBFA_PATH, args.ModuleFile)):
        print("ModuleFile path: {}".format(args.ModuleFile) +
              " is no exits or not the relative path for HBFA")
        os._exit(0)
    else:
